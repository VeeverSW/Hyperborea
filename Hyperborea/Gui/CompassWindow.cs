...
public unsafe class CompassWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        if (P.Enabled && layout != null)
        {
            ...
            ImGuiEx.Tooltip("传送到当前相机位置");
            ImGui.SameLine();
            ImGui.PushFont(UiBuilder.IconFont);
            ImGuiEx.ButtonCheckbox("\uf05b", ref C.FastTeleport);
            ImGui.PopFont();
            ImGuiEx.Tooltip("启用 Ctrl + 左键点击 传送到鼠标位置"); 
            ...
            ImGui.PopFont();
            ImGuiEx.Tooltip("启用坐骑飞行 (未骑乘时也可进行类似坐骑的飞行) 与 无碰撞模式 不兼容");
            if (C.ForcedFlight) P.Noclip = false;
            ImGui.SameLine();
            ImGui.PushFont(UiBuilder.IconFont);
            ImGuiEx.ButtonCheckbox("\uf6e2", ref P.Noclip);
            ImGui.PopFont();
            ImGuiEx.Tooltip("启用无碰撞模式 WASD - 移动, space - 向上移动, left shift - 向下移动");
            if (P.Noclip)
            {
                ImGui.SameLine();
            ...
            if (ImGui.BeginCombo("##fest", fests.IsNullOrEmpty()? "选择节日...":fests, ImGuiComboFlags.HeightLarge))
            {
                ImGui.SetNextItemWidth(150f);
